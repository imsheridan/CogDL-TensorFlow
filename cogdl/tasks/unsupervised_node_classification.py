...
from cogdl.data.in_memory_dataset import InMemoryDataset
...
class UnsupervisedNodeClassification(BaseTask):
    ...
    def train(self):
        ...
        if self.is_weighted:
            edges, weight = (
                self.data.edge_index.numpy().tolist(),
                self.data.edge_attr.numpy().tolist(),
            )
            G.add_weighted_edges_from(
                [(edges[i][0], edges[i][1], weight[0][i]) for i in range(len(edges))]
            )
        else:
            G.add_edges_from(self.data.edge_index.numpy().tolist())
